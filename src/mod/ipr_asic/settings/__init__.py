...
def get_auth(key: str) -> AuthPassword | None:
    """Returns AuthPassword of key if it exists."""
    try:
...
def get_auth_list(key: str) -> list[str]:
    """Returns stored alt & default passwords for key as list. Returns empty if key is not found."""
    a = get_auth(key)
    if a is not None:
        return a.as_list()
    return []
...
def set_alt_auth(key: str, alt: str) -> None:
    """Sets alt as alternative password for key."""
    a = get_auth(key)
    if a is not None:
        a.alt = alt
...
class Settings(BaseModel):
    ...
    api_function_timeout: float = Field(default=5.0)
